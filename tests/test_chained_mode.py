...
def get_apic_token():
    ...
    if response.status_code == 200:
        ...
        raise Exception("Failed to log in to APIC")

token = get_apic_token()


def get_subnets(bd_name, tenant_name):
    subnet_url =   f"{APIC_URL}/api/node/mo/uni/tn-{tenant_name}/BD-{bd_name}.json?query-target=subtree&target-subtree-class=fvSubnet"
    headers = {
    ...
    if response.status_code == 200:
        ...
        raise Exception("Failed to retrieve subnets")
    

def compare_subnets(subnets1, subnets2):
    ...
    def translate_terms(item, mapping):
    ...
    def normalize_subnet(subnet):
        if 'fvSubnet' in subnet:
            ...
            return {
                'subnet': attrs['ip'],
                'scope': translate_terms(attrs.get('scope', '').split(','), scope_mapping),  # Convert and translate scopes
                'control': translate_terms(attrs.get('ctrl', '').split(','), control_mapping)  # Convert and translate controls
            }
        else:
            return {
    ...
    normalized_subnets1 = [normalize_subnet(s) for s in subnets1]
    normalized_subnets2 = subnets2

    normalized_subnets1_sorted = sorted(normalized_subnets1, key=lambda ns: ns['subnet'])
    normalized_subnets2_sorted = sorted(normalized_subnets2, key=lambda ns: ns['subnet'])
  
    def compare_scope(ns1,ns2):
        scope1 = ns1.get('scope', [])
        scope2 = ns2.get('scope', [])
    ...
    def compare_control(ns1,ns2):            
        control1 = ns1.get('control', [])
        control2 = ns2.get('control', [])
    ...
    for ns1, ns2 in zip(normalized_subnets1_sorted, normalized_subnets2_sorted):
        if ns1['subnet'] != ns2['subnet']:
            return False    
        control = compare_control(ns1,ns2)
        scope = compare_scope(ns1,ns2) 
...
@pytest.mark.usefixtures("clean_gen_templates")
def test_contract_to_epg_mapping_from_k8s_to_aci_1(base_fixture):
    kapi, pods = KubeAPI(), {}
    vlans = get_vlans_from_fabricvlanpool(kapi, 'default')
    assert vlans, ("no VLAN information present in fabricvlanpool")
    ...
    try:
        # subnet_1 = [{'subnet': '10.5.0.1/24'}]
        # subnet_2 = [{ 'subnet': '10.6.0.1/24', 'control': ['querier-ip'], 'scope': ['shared-between-vrfs']}] 
        ...
        epg_details = [{
            'epg_name': 'test_epg1',
            'bd_name': 'test_bd1',
            'subnets': subnet_1, #str(ipaddress.IPv4Network(subnet_1[0]['subnet'])[1]) + '/24',
            'provider_contract_name': ['allow_icmp'],
            'consumer_contract_name': ['allow_icmp'],
            'vlans': vlans[0]
        },
        {
            'epg_name': 'test_epg2',
            'bd_name': 'test_bd2',
            'subnets': subnet_2, #str(ipaddress.IPv4Network(subnet_2[0]['subnet'])[1]) + '/24',
            'provider_contract_name': ['allow_icmp'],
            'consumer_contract_name': ['allow_icmp'],
            'vlans': vlans[1]
        }]
        ...
        bd1_subnets1 = get_subnets(bd_name1, tenant_name)
        bd2_subnets2 = get_subnets(bd_name2, tenant_name)
        
        # Call the comparison function
        are_subnets_equal_1 = compare_subnets(bd1_subnets1, subnet_1)
        print(f"\nAre the subnet configurations equal in test_bd1\n\n? {are_subnets_equal_1}\n")
        assert are_subnets_equal_1, ("subnets created on APIC is not matching with subnets_1 under test_bd1")

        are_subnets_equal_2 = compare_subnets(bd2_subnets2, subnet_2)
        print(f"\nAre the subnet configurations equal in test_bd2\n\n? {are_subnets_equal_2}\n")
