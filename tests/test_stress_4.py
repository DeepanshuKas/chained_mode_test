...
def setup_module(module):
    lib.collect_profiling_data(test_name="test_stress_4", when="before-test-run")


def teardown_module(module):
    lib.collect_profiling_data(test_name="test_stress_4", when="after-test-run")
    lib.show_cpu_memory_usage_difference(test_name="test_stress_4")


@pytest.mark.usefixtures("clean_gen_templates")
def test_stresssnat_for_deployment(base_fixture, gen_template_name):
    """ Creates namespace, snatpolicy and deployment/pods.
        Validates SNAT ep file and snat files.
        Add more deployments/pods to the existing namespace & snatpolicy.
    """
    def create_namespaces_and_policies(no_of_policy_per_thread, 
                                       index,
                                       fixture,
                                       ns_dict,
                                       policy_dict):
        for _count in range(no_of_policy_per_thread):
            # Creating namespaces        
            ns_name = "stress-ns-%s-%s" % (index, _count)
            ...
            # Creating policies
            policy_name = "snatpolicy-%s-%s" % (index, _count)
            policy_ip = "%s.0.0.%s/32" % (10 + index, _count + 2)
            policy_label = "label-%s-%s" % (index, _count)
            ...
            ns_dict[index][_count] = ns_name
            policy_dict[index][_count] = policy


    def run_deployments(kapi, no_of_policy_per_thread, replica_count, 
                        worker_node, fixture, result, index, 
                        ns_dict, policy_dict):
        for _count in range(no_of_policy_per_thread):
            ns_name = ns_dict[index][_count]
            policy = policy_dict[index][_count]
    ...
    print("Number of replicas: " + str(REPLICA_COUNT))
    ...
    _threads = [None] * number_of_threads
    _results = [None] * number_of_threads
    ...
    for idx in range(number_of_threads):
        policy_dict[idx] = {}
        ns_dict[idx] = {}
    # Separated Threads for namespaces-policies creation and deployments 
    # creation due to a threading issue: Sometimes deployment creation was 
    # throwing error 'policy-name doesn't exists'.
    # Create namespaces and policies
    for idx in range(number_of_threads):
        _threads[idx] = Thread(target=create_namespaces_and_policies,
                                            args=(no_of_policy_per_thread,
                                                idx,
                                                base_fixture,
                                                ns_dict,
                                                policy_dict))
