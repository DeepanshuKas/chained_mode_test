...
def setup_module(module):
    lib.collect_profiling_data(test_name="test_stress_3", when="before-test-run")


def teardown_module(module):
    lib.collect_profiling_data(test_name="test_stress_3", when="after-test-run")
    lib.show_cpu_memory_usage_difference(test_name="test_stress_3")
...
def cleanup(kapi, d_pol):
    try:
        kapi.delete_object('snatpolicy', d_pol)
    except Exception as err:
        print("Err: ", err)
        print("Deletion failed for : ", d_pol)
