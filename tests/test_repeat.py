...
def test_multiple_policy_and_deployment(base_fixture, gen_template_name):
    ...
    def run_deployments(kapi, count, workers, worker_node, fixture, result, index):
        for _count in range(count):
            ...
            ns_name = "repeat-ns-%s-%s" % (num, _count)
            ...
            policy_name = "repeat-snatpolicy-%s-%s" % (num, _count)
            policy_ip = "%s.0.0.%s/32" % (10 + index, _count + 2)
            policy_label = "repeat-label-%s-%s" % (num, _count)
            ...
            for _iter in range(_iterations):
                ...
                deployment_name = "repeat-nginx-deploy-%s-%s-%s" % (
                    num, _count, _iterations)
                deploy_in = {'name': deployment_name,
                             'replicas': replicas,
                             'node': worker_node,
                             'namespace': ns_name}
                deploy_label = "repeat-label-%s-%s" % (index, _count)
                selector = {'name': deploy_label}
                svc_name = "repeat-tlb-nginx-svc-%s-%s-%s" % (
                    num, _count, _iterations)
    ...
    print("Creating test with policy count: " + str(POLICY_COUNT))
